...
awk -F, 'NR > 1' $INPUTCSV | #choose rows
while IFS="," read -r \
...
#createPatchDict and removes patches having `0` faces.
    ...
    awk -v p_inlet="$p_inlet" '{
          ...
          print $0}' $MASTERSCRIPTSPATH/bUpFormatterConical.sh > \
...
# postProcess -func CourantNo || exit;# -latestTiPme;
    simpleFoam -postProcess -func wallShearStress | tee \
...
tee logs/"${PWD##*/}"_flowrate.log || exit;
